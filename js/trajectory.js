...
function thirdlinearSmooth5(path) {
	...
	if(N >= 5) {
		...
		for(i = 2; i <= N - 3; i++) {
			path[0][0] = (-3.0 * ( path[i-2][0] + path[i+2][0]) + 12.0 * ( path[i-1][0] + path[i+1][0]) + 17.0 * path[i][0]) / 35.0;
		}
		...
		for(i = 2; i <= N - 3; i++) {
			path[0][1] = (-3.0 * ( path[i-2][1] + path[i+2][1]) + 12.0 * ( path[i-1][1] + path[i+1][1]) + 17.0 * path[i][1]) / 35.0;
		}
